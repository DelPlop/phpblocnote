...
class NoteRepository extends ServiceEntityRepository
{
    ...
    public function getNotesByCategoryAndUser(Category $category, ApplicationUser $user): array
    {
        $qb = $this
            ...
            ->setParameter('user', $user)
            ->addOrderBy('nc.position', 'asc')
            ->addOrderBy('n.id', 'desc');

        return $qb->getQuery()->getResult();
    }

    public function findImportantNotes(ApplicationUser $user): array
    {
        $qb = $this
            ->createQueryBuilder('n')
            ->join('n.categories', 'c')
            ->andWhere('n.user = :user')
            ->setParameter('user', $user)
            ->andWhere('n.isImportant = :criterion')
            ->setParameter('criterion', true)
    ...
    public function findNotesWithActionNeeded(ApplicationUser $user): array
    {
        $qb = $this
            ->createQueryBuilder('n')
            ->join('n.categories', 'c')
            ->andWhere('n.user = :user')
            ->setParameter('user', $user)
            ->andWhere('n.needsAction = :criterion')
            ->setParameter('criterion', true)
            ->addOrderBy('c.position', 'asc')
            ->addOrderBy('n.id', 'desc');
